...
class ReminderAdaptor (context: Context, private val list: List<Reminder>): BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        val row = inflater.inflate(R.layout.list_view_items,parent,false)
        row.item_message.text=list[position].message
        row.item_location.text=list[position].location
        ...
        if (list[position].time !=null){
            ...
            row.item_trigger.text = readableTime
        } else {
            row.item_trigger.text = "Start Time"
        }
